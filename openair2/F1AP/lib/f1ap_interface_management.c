...
f1ap_served_cell_info_t copy_f1ap_served_cell_info(const f1ap_served_cell_info_t *src)
{
  f1ap_served_cell_info_t dst = {
    ...
    .nr_pci = src->nr_pci,
    // .num_ssi = src->num_ssi,  // [MOCN] Keep for compatibility
    .num_plmn = src->num_plmn,  // [MOCN] NEW: Copy num_plmn
    .mode = src->mode,
  };

  // [MOCN] OLD CODE (global slice array) - kept for reference/rollback
